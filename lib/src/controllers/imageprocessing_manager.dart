...
typedef OnImageCaptured = void Function(Uint8List orignalImage,
    Uint8List imageWithBoundingBox, BoundingBox regionOfInterest);
...
class ImageProcessingManager {
  ...
  static process(XFile image, Map<String, double> positionOnScreen,
      Size screenDisplaySize, OnImageCaptured callback) async {
    ...
    Offset rectangleStartPoint =
        Offset(positionOnScreen['x'] ?? 0.0, positionOnScreen['y'] ?? 0.0);
    Offset rectangleEndPoint =
        Offset(positionOnScreen['w'] ?? 0.0, positionOnScreen['h'] ?? 0.0);

    // Get Captured Image Size
    Size capturedImageSize =
        await ImageController.getImageSizeFromBytes(rawImageBytes);
    ...
    Uint8List imageBytesWithRegion =
        await ImageController.drawOnImage(image, rectanglePoints);
